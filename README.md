...
- Executar **Release Please** para gerar PR de release, criar tags automaticamente e publicar releases.
- Empacotar artefatos (extensões e APK) após criação de tag.

Fluxo resumido:

1. Commits entram na `main`.
2. Workflow `release-please.yml` atualiza versão e CHANGELOG via PR de release.
3. Ao merge do PR de release, a action cria tag `vX.Y.Z` e release.
4. Workflow `build-and-publish.yml` é disparado na tag e publica artefatos.
...
- ✅ Planejamento estratégico e roadmap inicial
- ⏳ Implementação do core criptográfico
- ⏳ MVP da extensão browser
