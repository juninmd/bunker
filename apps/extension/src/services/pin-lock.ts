...
// A short PIN is brute-forceable offline, so its envelope lives only in memory and dies with the browser.
...
export async function setupPin(pin: string, masterPassword: string): Promise<void> {
  if (pin.length < MIN_PIN_LENGTH) throw new Error('PIN too short');
  const salt = crypto.getRandomValues(new Uint8Array(16));
  const encrypted = await encryptPayload({ masterPassword }, pin, salt, VAULT_KDF_ITERATIONS);
  await setSessionValues({ [PIN_KEY]: { salt: bytesToBase64(salt), encrypted, failures: 0 } });
}

export async function recoverMasterPassword(pin: string): Promise<string> {
