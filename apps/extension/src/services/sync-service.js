...
export class SyncService {
    ...
    async importCSV() {
        ...
        const csvFile = await this.driveService.findFile(this.CSV_FILE);
        if (!csvFile)
            throw new Error('CSV file not found in Drive');
        ...
        const imported = parsed.map(row => {
            ...
            if (item.type === 'note' && !row.grouping)
                item.grouping = 'Secure Notes';
            if (item.type === 'address' && !row.grouping)
                item.grouping = 'Endereços';
            return item;
    ...
    mergeCSV(localVault, importedItems) {
        ...
        importedItems.forEach((newItem) => {
            const existing = merged.find((i) => {
                ...
                if ((i.type || 'password') !== newItem.type)
                    return false;
                if (i.site !== newItem.site)
                    return false;
